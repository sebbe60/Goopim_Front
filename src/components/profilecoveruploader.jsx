...
const ProfileCoverUploader = (props) => {
  const [uploadedImageUrl, setUploadedImageUrl] = useState("");
  const [tooltipStatus, setTooltipStatus] = useState(false);

  const handleUploadSuccess2 = (result) => {
    console.log("result: ", result);
    const { secure_url } = result.info;
    setUploadedImageUrl(secure_url);
    props.setFormData((prevState) => ({
      ...prevState,
      profile_cover_url: secure_url,
    }));
  };
  ...
  return (
    <div className="absolute flext justify-center">
      <CldUploadWidget uploadPreset="i8ddzech" onUpload={handleUploadSuccess2}>
        {({ open }) => (
          <button
